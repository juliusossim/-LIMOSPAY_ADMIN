 ...
 */
export const copyText = async ({ str, callback }) => {
  if (stringDoesNotExist(str)) {
    return false;
  }
  await navigator.clipboard.writeText(str);
  const text = await navigator.clipboard.readText().then((res) => callback({ copied: res, str }));
  return text;
};
...
export const makeInitials = (names) => `${stringCaps(names.split(' ')[0][0])}${stringCaps(names.split(' ')[1][0])}`;
